...
io.on("connection", (socket) => {
    ...
    socket.on("message:delete", async ({ messageId, senderId, receiverId, type, chatType }) => {
        try {
    ...
    socket.on("sidebar:update", async ({ chatType, senderId, receiverId, chatListId, messageId }) => {
        ...
        if (chatType === "group") {
            ...
            const lastMessage = groupConversation?.messages.find((msg) => msg.id === messageId);
            if (lastMessage) {
        ...
        const lastValidMessage = messages.find((msg) => msg.deletedByMeId !== senderId && // deleted for me ignore
    ...
    socket.on("clearChat", async ({ senderId, receiverId, type, groupId, sidebarChatId }) => {
        let clearChat = null;
        if (type === "chat") {
    ...
    socket.on("memeberLastMsgSeenUpdate", async ({ messageSenderId, lastMessageId, memberId, groupId }) => {
