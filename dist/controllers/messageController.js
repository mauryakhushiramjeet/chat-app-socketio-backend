    ...
    try {
        ...
        const messages = message?.map((msg) => {
            if (msg?.replyToMessageId) {
                const replyMessages = message?.find((message) => message?.id === msg?.replyToMessageId);
                return {
                    ...msg,
                    replyMessage: {
                        text: replyMessages?.text,
                        file: replyMessages?.file && replyMessages?.file?.length > 0
                            ? replyMessages?.file
                            : null,
...
export const getAllMyMessages = async (req, res) => {
...
export const getGroupMessages = async (req, res) => {
    ...
    try {
        ...
        const whereGroupMessages = { groupId: Number(groupId) };
        const clearChatExist = await prisma.chatClear.findUnique({
            where: {
                userId_groupId: {
                    userId: Number(senderId),
        ...
        const messages = groupMessagesWithReplies.map((groupMsg) => {
            const replyMessages = groupMessagesWithReplies.find((message) => message?.id === groupMsg?.replyToMessageId);
...
export const sendMessage = async (req, res) => {
    ...
    const { clientMessageId, text, senderId, receiverId, type, replyToMessageId, replyMessageSenderId, } = req.body;
    try {
        if (!senderId || !receiverId) {
            return res.status(400).json({
                success: false,
        ...
        if (files && files?.length > 0) {
            const createdFile = await Promise.all(files.map((file) => prisma.file.create({
                data: {
...
export const sendGroupMessage = async (req, res) => {
    const files = req.files;
    const { groupId, message, messageSenderId, replyToMessageId, replyMessageSenderId, } = req.body;
    try {
        ...
        const isMember = group.groupMembers.some((user) => String(user.userId) === String(messageSenderId));
        if (!isMember) {
            return res.status(400).json({
                success: false,
...
export const editMessageFile = async (req, res) => {
    const { messageId, senderId, newText, receiverId, type, deletedFileId, groupId, fileExist, } = req.body;
    const files = req?.files;
    try {
        ...
        if (type === "chat") {
            ...
            const deletedFiles = (Array.isArray(deletedFileId) ? deletedFileId : [deletedFileId])
                .map((id) => Number(id))
                .filter((id) => !isNaN(id) && id > 0);
            if (deletedFiles.length > 0) {
                await Promise.all(deletedFiles.map((id) => prisma.file.update({
                    where: { id },
                    data: { isDeleted: true },
                })));
            }
            ...
            if (Array.isArray(files) && files.length > 0) {
                createFiles = await Promise.all(files?.map(async (f) => await prisma.file.create({
                    data: {
        ...
        if (type === "group") {
            ...
            if (files && files?.length !== 0) {
                createGroupFile = await Promise.all(files?.map(async (f) => await prisma.file.create({
                    data: {
                        fileName: f.originalname,
                        filePath: f.path,
                        fileType: f.mimetype,
            ...
            groupChat.group.groupMembers.forEach((user) => {
                const socketId = onlineUsers[user.userId];
                if (socketId) {
